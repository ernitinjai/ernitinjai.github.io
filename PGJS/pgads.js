...
pgads.AdEvent = function(eventType) {
	this.type = eventType;
	this.error = '';
	...
	this.getAd = function() {
		return pgads.AdsManager.adPodInfo[0];
	}

	this.isInline = function () {
		if(pgads.AdsManager.adPodInfo[0].inline != 'undefined') {
			return true;
		} else {
			return false;
		}
	...
	this.getTotalAds = function () {
		return pgads.AdsManager.adPodInfo.length;
	}
...
pgads.AdsManager =  function(player){
	...
	this.player.on('pause', function () {
		//alert('pause');
		var adEvent = new pgads.AdEvent(pgads.AdEvent.Type.CONTENT_PAUSE_REQUESTED);
	...
	this.player.on('error',function () {
		alert('asdsfdvfdvdf');
		var adEvent = new pgads.AdEvent(pgads.AdEvent.Type.AD_ERROR);
		pgads.getEventsCallback(adManagerObj.adsManagerLoaderEvents,pgads.AdEvent.Type.AD_ERROR)(adEvent);
	});

	this.player.on('play', function () {
		 var adEvent = new pgads.AdEvent(pgads.AdEvent.Type.STARTED);
	...
	this.player.on('fullscreenchange', function() {
		var adEvent = new pgads.AdEvent(pgads.AdEvent.Type.FULLSCREEN);
	...
	this.player.on('vpaid.AdLoaded', function() {
		var adEvent = new pgads.AdEvent(pgads.AdEvent.Type.LOADED);
	...
	this.player.on('vpaid.AdStopped', function() {
		//alert('adStopped');
		var adEvent = new pgads.AdEvent(pgads.AdEvent.Type.CONTENT_RESUME_REQUESTED);
	...
	this.player.on('vpaid.AdVideoFirstQuartile', function() {
		...
		var adEvent = new pgads.AdEvent(pgads.AdEvent.Type.FIRST_QUARTILE);
	...
	this.player.on('vpaid.AdVideoMidpoint', function() {
		var adEvent = new pgads.AdEvent(pgads.AdEvent.Type.MIDPOINT);
	...
	this.player.on('vpaid.AdVideoThirdQuartile', function() {
		var adEvent = new pgads.AdEvent(pgads.AdEvent.Type.THIRD_QUARTILE);
	...
	this.player.on('vpaid.AdVideoComplete', function() {
       var adEvent = new pgads.AdEvent(pgads.AdEvent.Type.COMPLETE);
	...
	this.player.on('vpaid.AdPaused', function() {
		var adEvent = new pgads.AdEvent(pgads.AdEvent.Type.PAUSED);
